...
def generate_matches(query, location, api_key):
    ...
    try:
        ...
        query_embedding = openai.Embedding.create(input=query, model=model_engine)
        ...
        distances = cdist(query_embedding.reshape(1, -1), embeddings, metric="cosine")[
            0
        ]
...
iface = gr.Interface(
    ...
    inputs=[
        ...
        gr.Dropdown(choices=["Boulder", "Denver"], label="Location"),
        gr.Textbox(label="OpenAI API key", placeholder="must have access to GPT-4"),
    ],
    outputs=["html"],
    examples=[
        [
